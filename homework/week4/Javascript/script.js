...
function loadData(){
	var jsonObj = JSON.parse(document.getElementById("rawdata").value)
	dataPoints = [];
...
// set mask canvas containing crosshair
mask = document.getElementById('mask');
boundaries = mask.getBoundingClientRect();
x_transformToMask = createTransform([boundaries.left + padding, boundaries.right - padding], [canv_xMin, canv_xMax]);
x_transformToCanvas = createTransform([canv_xMin, canv_xMax], [boundaries.left + padding, boundaries.right - padding]);
x_transformToDate = createTransform([canv_xMin, canv_xMax], [inp_xMin, inp_xMax]);
dateToDatepoint = createTransform([inp_xMin, inp_xMax], [0, dataPoints.length]);
y_transformToMask = createTransform([canv_yMax, canv_yMin], [boundaries.bottom - padding, boundaries.top + padding]);
y_transformToTemp = createTransform([canv_yMax, canv_yMin], [inp_yMin, inp_yMax]);
...
var drawCrossHair = function(e) {
	...
	crosshair_width = 10;
	var x = e.pageX;
	if (x_transformToMask(x) >= canv_xMin && x_transformToMask(x) < canv_xMax) {
		var y = y_transform(dataPoints[Math.floor(dateToDatepoint(x_transformToDate(x_transformToMask(x))))].y);
		mask.style.cursor="none";
		mask_ctx.beginPath();
		mask_ctx.moveTo(x_transformToMask(x),canv_yMin);
		mask_ctx.lineWidth = 2;
		mask_ctx.lineTo(x_transformToMask(x), y - crosshair_width);
		mask_ctx.stroke();
		mask_ctx.beginPath();
		mask_ctx.moveTo(x_transformToMask(x), y + crosshair_width);
		mask_ctx.lineWidth = 2;
		mask_ctx.lineTo(x_transformToMask(x), canv_yMax);
		mask_ctx.stroke();
		mask_ctx.beginPath();
		mask_ctx.moveTo(canv_xMin, y);
		mask_ctx.lineWidth = 2;
		mask_ctx.lineTo(x_transformToMask(x) - crosshair_width, y);
		mask_ctx.stroke();
		mask_ctx.beginPath();
		mask_ctx.moveTo(x_transformToMask(x) + crosshair_width, y);
		mask_ctx.lineWidth = 2;
		mask_ctx.lineTo(canv_xMax, y);
		mask_ctx.stroke();
		mask_ctx.beginPath();
		mask_ctx.arc(x_transformToMask(x), y, crosshair_width, 0, 2 * Math.PI, false);
		mask_ctx.lineWidth = 2;
		mask_ctx.stroke();
		mask_ctx.beginPath();
		mask_ctx.arc(x_transformToMask(x), y, crosshair_width / 3, 0, 2 * Math.PI, false);
		mask_ctx.strokeStyle = '#FF0000';
		mask_ctx.lineWidth = 2;
		mask_ctx.stroke();
		drawTooltip(e, x, y)
		}
...
var setVisible = function() {
	document.getElementById('tooltip_temp').style.visibility = 'visible';
	document.getElementById('tooltip_date').style.visibility = 'visible';
}

// set tooltip
var drawTooltip = function(e, x, y) {
	document.getElementById('tooltip_temp').style.visibility = 'hidden';
	document.getElementById('tooltip_date').style.visibility = 'hidden';
	var timer;
	clearTimeout(timer)
	timer = setTimeout(setVisible, 2000);
	document.getElementById('tooltip_temp').textContent = " " +
	Math.floor(dataPoints[Math.floor(dateToDatepoint(x_transformToDate(x_transformToMask(x))))].y/10) + " \xB0C";
	document.getElementById('tooltip_temp').style.top = y + 'px';
	document.getElementById('tooltip_temp').style.left = x + 100 + 'px';
	if (parseFloat(document.getElementById('tooltip_temp').style.left) > document.getElementById("canvas").width ){
		document.getElementById('tooltip_temp').style.left =  x - 100 - document.getElementById("tooltip_temp").offsetWidth + 'px';
	}
	tooltip_date =  new Date(x_transformToDate(x_transformToMask(x)));
	document.getElementById('tooltip_date').textContent = tooltip_date.getFullYear() + ' / ' +
	(tooltip_date.getMonth() + 1) + ' / ' + tooltip_date.getDate()
	document.getElementById('tooltip_date').style.transform = "rotate(90deg)";
	document.getElementById('tooltip_date').style.top =  y + 100 + 'px';
	document.getElementById('tooltip_date').style.left = x - document.getElementById("tooltip_date").offsetWidth +
	document.getElementById("tooltip_date").offsetHeight + 'px';
	if (parseFloat(document.getElementById('tooltip_date').style.top) + document.getElementById("tooltip_date").offsetHeight
	> y_transformToMask(y_transform(inp_yMin))){
		document.getElementById('tooltip_date').style.top =  y - 100 + 'px';
	}
}

window.addEventListener("load",function(){
	mask.addEventListener("mousemove", drawCrossHair);
});